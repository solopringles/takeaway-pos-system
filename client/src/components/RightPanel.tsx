...
const PRIMARY_CATEGORIES_ICONS = [
    { name: 'Chicken', icon: '🐔' }, { name: 'Beef', icon: '🐄' },
    { name: 'Pork', icon: '🐖' }, { name: 'Duck', icon: '🦆' },
    { name: 'Lamb', icon: '🐑' }, { name: 'King Prawn', icon: '🦐' },
    { name: 'Shrimp', icon: '🍤' }, { name: 'Fish', icon: '🐟' },
    { name: 'Mushroom', icon: '🍄' }, { name: 'Veg', icon: '🥦' },
    { name: 'Special', icon: '⭐' }, { name: 'Rice', icon: '🍚' },
    { name: 'Chips', icon: '🍟' }, { name: 'Btn 1', icon: '1️⃣' },
    { name: 'Btn 2', icon: '2️⃣' }, { name: 'Btn 3', icon: '3️⃣' },
];
...
const SECONDARY_CATEGORIES_PAGES = [
    [
      ...
      { zh: '古老', en: 'Sweet & Sour' }, { zh: '辣蒜', en: 'Chilli Garlic' }, { zh: '京式', en: 'Peking' }, { zh: '腰果', en: 'Cashew Nuts' }, { zh: '黑召', en: 'Black Pepper' }, { zh: '柠檬/梅', en: 'Lemon/Plum' }, { zh: '菠萝', en: 'Pineapple' },
...
const PrimaryHorizontalButton = ({ icon, isActive, onClick }: { icon: string, isActive: boolean, onClick: () => void }) => (
    <button
        onClick={onClick}
...
const RightPanel: React.FC<RightPanelProps> = ({ menuItems, onAddItem, onOpenMenuRef }) => {
    ...
    const filteredResults = useMemo(() => {
        // --- FIX: Always return a new array to prevent state mutation ---
        if (!selectedPrimary && !selectedSecondary) {
            // By returning a shallow copy, we ensure the original menuItems array in App.tsx is never mutated.
            return [...menuItems];
        }

        return menuItems.filter(item => {
            const primaryMatch = selectedPrimary ? item.primaryCategory === selectedPrimary : true;
            const secondaryMatch = (selectedSecondary && selectedSecondary !== 'Show All') ? item.secondaryCategory.includes(selectedSecondary) : true;
    ...
    const selectedItem = useMemo(() => filteredResults.find(i => i.id === selectedResultId), [filteredResults, selectedResultId]);
        ...
        <div className="w-[62%] bg-gray-300 flex flex-col gap-2">
                        ...
                        <tbody>
                            {filteredResults.map(item => (
                                <tr
                                    ...
                                    onDoubleClick={() => handleAddItemAndClear(item)}
                                    className={`cursor-pointer ${selectedResultId === item.id ? 'bg-blue-600 text-white' : 'hover:bg-yellow-200'}`}
                                >
                                    <td className="p-1 font-mono">{item.id}</td>
                                    <td className="p-1">{item.name.en}</td>
                                    <td className="p-1">{item.name.zh}</td>
                                    <td className="p-1 font-mono text-right">£{(item.price ?? 0).toFixed(2)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            <div className="flex-grow flex flex-col">
                <div className="flex-shrink-0 flex gap-2 h-8 mb-2">
                    <button className="bg-gray-300 border-2 border-r-gray-500 border-b-gray-500 border-l-gray-100 border-t-gray-100 px-2">F8 - Dish Type 餐款入单</button>
                    <button onClick={onOpenMenuRef} className="bg-gray-300 border-2 border-r-gray-500 border-b-gray-500 border-l-gray-100 border-t-gray-100 px-2">Menu Ref 餐号入单</button>
                </div>
